...
function StepEditorBase({
  ...
  const updateEdgeLabel = (edgeId: string, val: string) => {
    onModelChange({
      ...model,
      edges: model.edges.map((e) => (e.id === edgeId ? { ...e, label: val || undefined } : e)),
    });
  };
  ...
  const addBranch = () => {
    if (branchMode === 'new') {
      ...
      const newEdge: DiagramEdge = {
        ...
        from: nodeId,
        to: newId,
        label: branchEdgeLabel.trim() || undefined,
      };
      onModelChange({
        ...model,
  ...
  const addAnswer = () => {
    ...
    onModelChange({
      ...
      nodes: model.nodes.map((n) =>
        n.id === nodeId ? { ...n, metadata: { ...(n.metadata ?? {}), answers: updated } } : n,
      ),
    });
    setNewAnswer('');
    setAddingAnswer(false);
  ...
  const moveAnswer = (idx: number, dir: -1 | 1) => {
    ...
    onModelChange({
      ...model,
      nodes: model.nodes.map((n) =>
        n.id === nodeId ? { ...n, metadata: { ...(n.metadata ?? {}), answers: arr } } : n,
      ),
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>
          {isQuestion ? 'Question Editor' : variant === 'journey' ? 'Step Editor' : 'Step Editor'}
        </span>
      </div>
          ...
          >
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0, padding: '8px 10px' }}>
                    ...
                    {connected && targetNode && (
                      <div style={{ fontSize: 11, color: accentColor, opacity: 0.85 }}>
                        → {targetNode.label}
                      </div>
                    )}
                    {!connected && (
                      <div style={{ fontSize: 10, color: tt.textMuted, fontStyle: 'italic' }}>
                        drag port to connect
                      </div>
                    )}
                  </div>
                  <div
            ...
            {addingAnswer ? (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={addAnswer} style={addBtnStyle}>
                    Add Answer
                  </button>
                  <button
                    onClick={() => {
            ...
            {answers.length > 0 && (
              ...
              >
                ...
                <div style={{ fontSize: 11, color: tt.textSecondary, lineHeight: 1.5 }}>
                  Hover the question node on the canvas — drag an answer's port dot to any other
                  node.
                </div>
              </div>
            )}
          ...
          >
            ...
            {addingBranch ? (
              ...
              >
                ...
                />
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={addBranch} style={addBtnStyle}>
                    Add {branchTerm}
                  </button>
                  <button onClick={() => setAddingBranch(false)} style={cancelBtnStyle}>
                    Cancel
                  </button>
                </div>
              </div>
